...
//				try {
//					new mainInintials();
//					SubGroupPanel frame = new InsertSubGroup();
...
//	}
	...
	public SubGroupPanel(String title) {
		...
		if(title.equals("VIEW"))
		{
			...
			setVisible(true);
			try {
				sf = new SearchFrame("SUBGROUPMASTER");
				sf.setVisible(true);
	...
	void InsertSubGroup()
	{
			...
			public void keyPressed(KeyEvent e) {
						try {
							...
							{
								mi.buildTableModel(mi.selectDataTable("ALIAS,NAME","GroupMaster"));	
								if(mi.getAliasNameMap().keySet().contains(groupField.getText()))
								{
									saveButton.requestFocus();
									groupField.setText(mi.getAliasNameMap().get(groupField.getText()));
								}
								else {
									saveButton.requestFocus();
										sf = new SearchFrame("GroupMaster");
										sf.setVisible(true);
										 sf.addWindowListener(new WindowAdapter(){  
									            public void windowClosed(WindowEvent e) {  
